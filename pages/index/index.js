...
var casualNav = config.casualNavUrl
var recommendArticle = config.recommendArticle
...
Page({
    ...
    getRecommendArticle () {
        wx.request({
            url: recommendArticle + '?key=Recommend',
            method: "GET",
            success: res => {
                const { data } = res
                console.log(data, 'recommends')
                this.setData({
                    postsstickyList: data.data.recommends
                })
            }
        })
    },
